...
# The real pytron_native bridge is bound at import time in window.py / system.py.
# Patching sys.modules after the fact has no effect on already-bound names.
# We must patch the module-level variable directly.
WIN_OS = "pytron.platforms.windows_ops.window.pytron_native"
SYS_OS = "pytron.platforms.windows_ops.system.pytron_native"
...
def test_window_minimize_noop_when_no_hwnd():
    with (
        patch("pytron.platforms.windows_ops.window.get_hwnd", return_value=0),
        patch(WIN_OS, None),
...
def test_window_close_ctypes(hwnd_window, no_pytron_native_window):
...
def test_window_show_ctypes(hwnd_window, no_pytron_native_window):
    with (
...
def test_window_hide_ctypes(hwnd_window, no_pytron_native_window):
...
def test_set_always_on_top_enable_ctypes(hwnd_window, no_pytron_native_window):
...
def test_set_always_on_top_disable_ctypes(hwnd_window, no_pytron_native_window):
    with patch.object(window.user32, "SetWindowPos") as mock_swp:
...
def test_set_fullscreen_enable_calls_setwindowlongw(
    hwnd_window, no_pytron_native_window
):
    with (
        patch.object(window.user32, "GetWindowRect"),
...
def test_set_fullscreen_enable_calls_setwindowpos(hwnd_window, no_pytron_native_window):
    with (
        patch.object(window.user32, "GetWindowRect"),
        patch.object(window.user32, "GetWindowLongW", return_value=0),
...
def test_set_fullscreen_disable_restores_style(hwnd_window, no_pytron_native_window):
...
def test_set_bounds_ctypes(hwnd_window, no_pytron_native_window):
    with patch.object(window.user32, "SetWindowPos") as mock_swp:
...
def test_make_frameless_strips_caption(hwnd_window, no_pytron_native_window):
    with (
        patch.object(
...
def test_center_calls_setwindowpos(hwnd_window, no_pytron_native_window):
    with (
        patch.object(window.user32, "GetWindowRect"),
...
def test_start_drag_ctypes(hwnd_window, no_pytron_native_window):
    with (
        patch.object(window.user32, "ReleaseCapture") as mock_rc,
...
def test_is_visible_true(hwnd_window, no_pytron_native_window):
    with patch.object(window.user32, "IsWindowVisible", return_value=True):
        assert window.is_visible("w") is True


def test_is_visible_false(hwnd_window, no_pytron_native_window):
...
def test_message_box_ctypes(hwnd_system, no_pytron_native_system):
...
def test_message_box_different_styles(hwnd_system, no_pytron_native_system):
    for style in (0, 1, 4):
        with patch.object(system.user32, "MessageBoxW", return_value=1) as mock_mb:
...
def test_system_notification_noop_when_no_hwnd(no_pytron_native_system):
    with (
        patch("pytron.platforms.windows_ops.system.get_hwnd", return_value=0),
...
def test_set_window_icon_skips_missing_file(hwnd_system, no_pytron_native_system):
...
def test_open_file_dialog_ctypes_calls_getopenfn(hwnd_system, no_pytron_native_system):
    with patch.object(
...
def test_save_file_dialog_ctypes_returns_value(hwnd_system, no_pytron_native_system):
...
def test_open_folder_dialog_ctypes_returns_path_from_pidl(
    ...
    with (
        patch.object(system.shell32, "SHBrowseForFolderW", return_value=fake_pidl),
...
def test_set_launch_on_boot_rust_path():
    mock_pytron_native = MagicMock()
    mock_pytron_native.set_launch_on_boot.return_value = True
    with patch(SYS_OS, mock_pytron_native):
        result = system.set_launch_on_boot("MyApp", "C:\\app.exe", True)
    assert result is True
    mock_pytron_native.set_launch_on_boot.assert_called_with(
        "MyApp", "C:\\app.exe", True
    )
...
def test_set_launch_on_boot_enable_via_winreg():
    ...
    mock_dep = MagicMock()
    mock_dep.pytron_native = None  # attr available but None → AttributeError on call
    ...
    with (
        patch(SYS_OS, None),
        patch.dict(_sys.modules, {"pytron.dependencies": mock_dep}),
...
def test_set_taskbar_progress_noop_when_no_pytron_native(
    hwnd_system, no_pytron_native_system
):
    # without pytron_native there is no ctypes path, just a silent return
...
def test_set_clipboard_text_falls_back_without_pytron_native():
    buffer = ctypes.create_unicode_buffer("hello")
    mock_user32 = MagicMock()
    mock_kernel32 = MagicMock()
    mock_user32.OpenClipboard.return_value = True
    mock_user32.SetClipboardData.return_value = 1
    mock_kernel32.GlobalAlloc.return_value = 123
    mock_kernel32.GlobalLock.return_value = ctypes.addressof(buffer)

    with (
        patch(SYS_OS, None),
        patch.object(system, "user32", mock_user32),
        patch.object(system, "kernel32", mock_kernel32),
    ):
        result = system.set_clipboard_text("hello")
        assert result is True


def test_set_clipboard_text_falls_back_on_native_exception():
    mock_os = MagicMock()
    buffer = ctypes.create_unicode_buffer("hello")
    mock_user32 = MagicMock()
    mock_kernel32 = MagicMock()
...
def test_get_clipboard_text_falls_back_without_pytron_native():
    buffer = ctypes.create_unicode_buffer("hello")
    mock_user32 = MagicMock()
    mock_user32.OpenClipboard.return_value = True
    mock_user32.GetClipboardData.return_value = ctypes.addressof(buffer)

    with patch(SYS_OS, None), patch.object(system, "user32", mock_user32):
        result = system.get_clipboard_text()
        assert result == "hello"


def test_get_clipboard_text_falls_back_on_native_exception():
    mock_os = MagicMock()
    buffer = ctypes.create_unicode_buffer("hello")
    mock_user32 = MagicMock()
    ...
    with patch(SYS_OS, mock_os), patch.object(system, "user32", mock_user32):
        result = system.get_clipboard_text()
        assert result == "hello"
...
def test_set_utility_window_enable_sets_toolwindow(
    hwnd_window, no_pytron_native_window
):
    with (
        patch.object(window.user32, "GetWindowLongW", return_value=0),
...
def test_set_utility_window_disable_sets_appwindow(
    hwnd_window, no_pytron_native_window
):
    with (
        patch.object(window.user32, "GetWindowLongW", return_value=0),
...
def test_set_utility_window_calls_setwindowpos(hwnd_window, no_pytron_native_window):
    with (
        patch.object(window.user32, "GetWindowLongW", return_value=0),
        patch.object(window.user32, "SetWindowLongW"),
        patch.object(window.user32, "SetWindowPos") as mock_swp,
