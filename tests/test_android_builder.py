...
def test_fetch_prebuilt_wheel_method_success(builder, tmp_path):
    with (
        ...
        patch("os.listdir", return_value=["pkg-1.0-android_24_aarch64.whl"]),
    ):
...
def test_build_wheel_uses_prebuilt(builder, tmp_path):
    ...
    # Mock repair_wheel
    with (
        patch.object(builder, "_fetch_prebuilt_wheel", return_value=True) as mock_fetch,
        patch.object(builder, "repair_wheel") as mock_repair,
        patch("os.listdir", return_value=["numpy.whl"]),
    ):
...
def test_build_wheel_fallback_to_build(builder, tmp_path):
    ...
    with (
        ...
        patch.object(builder, "_ensure_tools") as mock_ensure,
    ):
