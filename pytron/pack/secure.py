...
class MetadataEditor:
    ...
    def _update_windows(self, binary_path, icon_path, settings):
        ...
        try:
            ...
            patch_map = {
                "PYTRON_PLACEHOLDER_COMPANY_NAME_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX": settings.get("author", "Pytron User"),
                "PYTRON_PLACEHOLDER_FILE_DESCRIPTION_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX": settings.get("description", "Pytron Secure App"),
                ...
                "PYTRON_PLACEHOLDER_LEGAL_COPYRIGHT_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX": settings.get("copyright", f"Copyright © {settings.get('author', '2026')}"),
                ...
                "PYTRON_PLACEHOLDER_PRODUCT_NAME_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX": settings.get("title", binary_path.stem),
            }
            ...
            for placeholder, value in patch_map.items():
                ...
                if idx != -1:
                    ...
                    log(f"Warning: Placeholder not found in binary: {placeholder[:30]}...", style="warning")
            
            with open(binary_path, 'wb') as f:
                f.write(content)
                
            log(f"Binary Metadata Patch Applied ({patched_count} fields)", style="success")
    ...
    def _update_macos(self, binary_path, icon_path, settings, dist_dir):
        ...
        if icon_path and os.path.exists(icon_path):
            if str(icon_path).endswith(".icns"):
                ...
                log("Warning: macOS requires .icns format for bundle icons.", style="warning")
...
def append_key_footer(binary_path, key_bytes, settings):
    ...
    try:
        ...
        log(f"Sealed {binary_path.name}: Embedded Shield Key (40 bytes).", style="success")
        return True
    except Exception as e:
...
def run_secure_build(args, script, out_name, settings, app_icon, package_dir, add_data, progress, task):
    ...
    try:
        ...
        progress.update(task, description="Bootloader: Preparing Environment...", completed=10)
...
"""
        ...
        for item in add_data:
            if os.pathsep in item:
                ...
                spec_datas.append((str(Path(src).resolve()).replace("\\", "/"), dest.replace("\\", "/")))
...
"""
        ...
        progress.update(task, description="Bootloader: Bundling Runtime...", completed=30)
        
        build_cmd = [get_python_executable(), "-m", "PyInstaller", "--noconfirm", spec_path.name]
        env = os.environ.copy()
        env["PYTHONPATH"] = str(script_dir.resolve()) + os.pathsep + env.get("PYTHONPATH", "")
        
        ret = run_command_with_output(build_cmd, cwd=str(build_dir), env=env, style="dim")
        if ret != 0:
        ...
        progress.update(task, description="Bootloader: Encrypting Payload...", completed=60)
        ...
        progress.update(task, description="Bootloader: Deploying Loader...", completed=80)
        
        ext = ".exe" if sys.platform == "win32" else ""
        loader_name = f"pytron_rust_bootloader{ext}"
        precompiled_bin = Path(package_dir) / "pytron" / "pack" / "secure_loader" / "bin" / loader_name
        
        if not precompiled_bin.exists():
            log("Error: Native loader binary not found in pytron/pack/secure_loader/bin/", style="error")
            return 1
        ...
        final_loader = apply_metadata_to_binary(final_loader, app_icon, settings, final_dist)
        ...
        if getattr(args, "installer", False):
            progress.update(task, description="Shield: Building Installer...", completed=95)
