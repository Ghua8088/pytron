    ...
    with _NATIVE_CACHE["lock"]:
        ...
        if getattr(sys, "frozen", False):
            ...
            if hasattr(sys, "_MEIPASS"):
                ...
                search_paths.append((PRIORITY_FROZEN_MEIPASS, os.path.join(sys._MEIPASS, "pytron", "dependencies")))
            
            # Executable Dir (Nuitka / OneDir)
            exe_dir = os.path.dirname(os.path.abspath(sys.executable))
            search_paths.append((PRIORITY_FROZEN_INTERNAL, os.path.join(exe_dir, "_internal", "pytron", "dependencies")))
            
            # Also check direct executable root for flat layouts
            search_paths.append((PRIORITY_FROZEN_ROOT, os.path.join(exe_dir, "dependencies")))
            
        else:
            # DEV PRIORITY
            # Check relative to this file (pytron/utils.py -> pytron/dependencies)
            base_utils = os.path.dirname(os.path.abspath(__file__))
            search_paths.append((PRIORITY_DEV_LOCAL, os.path.join(base_utils, "dependencies")))
        ...
        for priority, path in search_paths:
            ...
            if os.path.exists(pyd_path):
                try:
                    spec = importlib.util.spec_from_file_location("pytron.dependencies.pytron_native", pyd_path)
        ...
        if not candidate_modules:
            try:
                ...
                import importlib
                try:
                    native_pkg = importlib.import_module(".pytron_native", package="pytron.dependencies")
                    path = getattr(native_pkg, "__file__", "package_import")
                    candidate_modules.append((PRIORITY_PACKAGE_FALLBACK, path, native_pkg))
                except:
                    pass
            except: 
                pass
