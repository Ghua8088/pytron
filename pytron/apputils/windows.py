            ...
            try:
                if sys.platform == "win32":
                    ...
                    def _drop_wrapper(files):
                        self.thread_pool.submit(self._on_file_drop_callback, window, files)
                    ...
                    self.logger.warning("Native Drag & Drop not yet implemented for macOS.")
            except Exception as e:
            ...
            try:
                if inspect.iscoroutinefunction(callback):
                    ...
                    try:
                        ...
                        self.logger.error(f"Async on_exit callback timed out or failed: {e}")
                else:
                    callback()
            except Exception as e:
    ...
    def register_protocol(self, scheme="pytron"):
        try:
            ...
            if impl and impl.register_protocol(scheme):
                ...
                self.logger.warning(f"Failed to register protocol: {scheme}:// (Not supported or failed on this OS)")
        except Exception as e:
