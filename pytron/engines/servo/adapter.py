...
logger = logging.getLogger("Pytron.ServoAdapter")


class ServoIPCServer:
    """
    A robust Platform-Native IPC server for the Servo Engine.
    ...
    def listen(self):
        ...
        if self._native:
            try:
                ...
                logger.info(
                    f"ServoNative IPC (Native) listening on: {self.pipe_path_base}"
                )
                self._native.wait_for_connection()
                self.connected = True
                logger.info("ServoNative Shell connected via Native Pipes")
                return
    ...
    def _listen_windows(self, uid):
        ...
        self.listening_event.set()
        logger.info(
            f"ServoNative IPC listening on Dual Pipes: {self.pipe_path_base} (-in/-out)"
        )
        ...
        self.connected = True
        logger.info("ServoNative Shell connected via Dual Pipes")
    ...
    def _listen_unix(self, uid):
        ...
        self.listening_event.set()
        logger.info(f"ServoNative IPC listening on UDS: {self.pipe_path_base}")

        self.conn, addr = self._sock.accept()
        self.conn.setblocking(True)
        self.connected = True
        logger.info(f"ServoNative Shell connected from {addr}")
...
class ServoAdapter:
    ...
    def start(self):
        ...
        logger.info(f"Spawning ServoNative Process (IPC): {' '.join(cmd)}")
        ...
        try:
            while True:
                ...
                if prefix == "STDOUT":
                    ...
                    if content.startswith("[ServoNative-Shell]"):
                        logger.info(content)
                    else:
                        logger.debug(f"[Electron] {content}")
                else:
                    # STDERR usually contains Servo warnings
    ...
    def _on_message(self, msg):
        ...
        logger.debug(f"ServoNative Received: {msg_type} -> {payload}")

        if msg_type == "lifecycle" and payload == "app_ready":
            logger.info("ServoNative Handshake (app_ready) received. Initiating flush.")
            self.ready = True
