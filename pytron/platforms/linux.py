...
class LinuxImplementation(PlatformInterface):
    ...
    def register_pytron_scheme(self, w, callback):
        ...
        try:
            ...
            if settings:
                self.libwebkit.webkit_settings_set_allow_file_access_from_file_urls(settings, 1)
                self.libwebkit.webkit_settings_set_allow_universal_access_from_file_urls(settings, 1)
        except Exception: 
            pass
        ...
        try:
            ...
            def scheme_handler(request, user_data):
                try:
                    # Get URI
                    self.libwebkit.webkit_uri_scheme_request_get_uri.argtypes = [ctypes.c_void_p]
                    self.libwebkit.webkit_uri_scheme_request_get_uri.restype = ctypes.c_char_p
                    uri_bytes = self.libwebkit.webkit_uri_scheme_request_get_uri(request)
                    ...
                    if uri.startswith("pytron://"):
                        ...
                        if data:
                             ...
                             self._gio.g_memory_input_stream_new.restype = ctypes.c_void_p
                             stream = self._gio.g_memory_input_stream_new()
                             
                             self._gio.g_memory_input_stream_add_data.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_longlong, ctypes.c_void_p]
                             ...
                             self._gio.g_memory_input_stream_add_data(stream, c_data, len(data), None)
                             
                             # Finish Request
                             # webkit_uri_scheme_request_finish (req, stream, length, mime)
                             self.libwebkit.webkit_uri_scheme_request_finish.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_longlong, ctypes.c_char_p]
                             self.libwebkit.webkit_uri_scheme_request_finish(request, stream, len(data), mime.encode('utf-8'))
            ...
            self.libwebkit.webkit_web_context_register_uri_scheme.argtypes = [
                ctypes.c_void_p, ctypes.c_char_p, URISchemeCallback, ctypes.c_void_p, ctypes.c_void_p
            ]
            
            self.libwebkit.webkit_web_context_register_uri_scheme(ctx, "pytron".encode('utf-8'), c_callback, None, None)
